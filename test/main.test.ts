...
import {convertToLocal, convertToServer} from "@/main";
...
describe('convertToLocal',() => {
    ...
    test('服务器返回的东八区时间经过转换后仍然为东八区时间',() => {
        expect(convertToLocal('2023/4/23 16:28:00').server().format('HH')).toBe('16')
    })
    test('带有时区信息的时间不受影响',() => {
        expect(convertToLocal(new Date()).format('HH')).toEqual(dayjs().format('HH'))
    })
})
